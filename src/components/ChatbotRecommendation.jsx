...
const ChatbotRecommendation = ({ formData, onSuggestionApply }) => {
  ...
  const generateSuggestions = (data) => {
    ...
    if (data.title && data.title.length < 10) {
      newSuggestions.push({
        type: "title",
        content:
          "Your title is quite short. Consider adding more details to grab attention.",
      });
    }
    ...
    if (data.description && data.description.length < 100) {
      newSuggestions.push({
        type: "description",
        content:
          "Adding more details to your description can help donors understand your cause better.",
      });
    }
    ...
    if (data.goal && Number.parseInt(data.goal) > 10000) {
      newSuggestions.push({
        type: "goal",
        content:
          "For larger goals, consider breaking down how the funds will be used to build trust.",
      });
    }
    ...
    if (data.category === "Emergency") {
      newSuggestions.push({
        type: "emergency",
        content:
          "For emergency campaigns, adding time-sensitive information can create urgency.",
        example:
          "We need to act within the next 48 hours to provide immediate relief.",
      });
    }
    ...
    if (!data.image) {
      newSuggestions.push({
        type: "image",
        content:
          "Campaigns with images receive 2x more donations. Add a compelling photo.",
      });
    }
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      let responseContent =
        "I'm sorry, I don't have enough information to help with that specific question.";

      const lowerCaseInput = inputValue.toLowerCase();
      if (
        lowerCaseInput.includes("title") ||
        lowerCaseInput.includes("headline")
      ) {
        responseContent =
        ...
        lowerCaseInput.includes("description") ||
        ...
        lowerCaseInput.includes("successful")
      ) {
        responseContent =
  ...
  const handleApplySuggestion = (suggestion) => {
    ...
    setMessages((prev) => [
      ...
      {
        role: "assistant",
        content: `Great! I've provided a suggestion for your ${
          suggestion.type
        }. ${suggestion.content} ${
